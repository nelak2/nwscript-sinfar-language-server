...
export default class DiagnoticsProvider extends Provider {
  constructor(server: ServerManager) {
    ...
    this.server.liveDocumentsManager.onDidSave(
      async (event) => await this.asyncExceptionsWrapper(async () => await this.publish(event.document.uri)),
    );
  ...
  private async publish(uri: string): Promise<void> {
    ...
    for (const child of children) {
      const fileUri = (await this.server.documentsCollection.get(child))?.uri;
      if (fileUri) {
        files[fileUri] = [];
        uris.push(fileUri);
      }
  ...
  public async processDocumentsWaitingForPublish() {
    return await Promise.all(
      this.server.documentsWaitingForPublish.map(
        async (uri) => await this.asyncExceptionsWrapper(async () => await this.publish(uri)),
      ),
    );
  }
}
