import { Utp } from "../editorProviders/resData";
import { buildTextField, buildLabelColumn, buildDiv, ButtonType } from "./utils";

export class nwnPLCInventory extends HTMLElement {
  _content!: Utp;
  ...
  private addClickEventHandler(e: Event, newValue: string) {
    ...
    const textField = document.getElementById("InventoryAdd") as HTMLInputElement;
    if (textField) textField.value = "";

    this.refreshList();

    if (oldValue === newValue) {
      this.dispatchEvent(new CustomEvent("PLCInventory_change", { detail: { field: this.id, undefined, newValue } }));
    }
  }
  ...
  private deleteClickEventHandler(e: Event, index: number) {
    ...
    if (oldValue) {
      this.dispatchEvent(
        new CustomEvent("PLCInventory_change", {
          detail: { field: "PLCInventory_item_" + index.toString(), oldValue, newValue: undefined },
        }),
      );
    }
  }
  ...
  private changeEventHandler(e: Event, index: number) {
    ...
    this.dispatchEvent(
      new CustomEvent("PLCInventory_change", {
        detail: { field: "PLCInventory_item_" + index.toString(), oldValue, newValue },
      }),
    );
  }
  ...
  private buildInventoryListRow(): HTMLDivElement {
    const listLabelCol = buildLabelColumn("Inventory Items", "PLCInventory");
  ...
  private buildInventoryAddRow(): HTMLDivElement {
    const inventoryAddLabelCol = buildLabelColumn("Add Item", "PLCInventoryAdd_txt");
    const inventoryAddInputCol = buildTextField({
      id: "PLCInventoryItemAdd",
      value: undefined,
      disabled: false,
      style: undefined,
      className: undefined,
